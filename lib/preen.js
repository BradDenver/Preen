...
var filterList; // an optional argument to supply filters from build systems (Gulp or Grunt)
...
function preen(options, callback) {
  ...
  try {
    if(!filterList)
      ...
      bowerJSON = filterList;
  }
  catch(err) {
    //logger.error('Error trying to read '+bower.config.json, err);
    logger.error('Error trying to read provided configuration', err);
    return;
  }
